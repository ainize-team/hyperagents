    ...
    async init(): Promise<void> {
      try {
        ...
        try {
          ...
          const memoryIndexContent = await this.client.get(this.memoryId);
          
          if (memoryIndexContent && memoryIndexContent.message) {
            try {
              const indexData = JSON.parse(memoryIndexContent.message);
              if (indexData.rootCid) {
                this.rootCid = indexData.rootCid;
              ...
              console.error('AINetworkDAGMemory: Error parsing memory index:', parseError);
            }
          }
        } catch (error) {
