...
class Agent {
  ...
  private async vote(result: string): Promise<void> {
    ...
    if (this.privateKey?.has(PrivateKeyType.ETH)) {
      ...
      console.log(`**agent ${this.name} voted proposal:${proposalId} with ethPrivateKey:${truncatedKey}`);
    }
  }
  ...
  private async trade(output: string): Promise<void> {
    ...
    if (this.privateKey?.has(PrivateKeyType.CDPKEY) && this.privateKey.has(PrivateKeyType.CDPNAME)) {
